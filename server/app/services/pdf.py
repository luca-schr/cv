"""Export PDF via Pandoc + WeasyPrint ΓÇö remplissage A4 une page."""
...
ROOT = settings.config_dir.parent  # server/
...
def _maximize_fill(
    ...
    *,
    ...
    """Agrandit au maximum (├⌐chelle + espacements) tout en restant sur 1 page."""
...
def export_pdf_result(
    ...
    *,
    ...
    progress("prepare", "Préparation du document…", 10)
    ...
    try:
        progress("convert", "Conversion Pandoc + WeasyPrint…", 35)
        progress("layout", "Ajustement mise en page A4…", 55)
        result = _optimize_layout(md_path, pdf_path, env=env)
        progress("pages", f"Contrôle pages — {result.page_count} page(s)", 85)
        progress("done", "PDF prêt", 100)
        return result
    except FileNotFoundError:
        raise HTTPException(
            503,
            "Pandoc non install├⌐. Installe Pandoc + WeasyPrint pour exporter en PDF.",
        ) from None
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or exc.stdout or str(exc)).strip()
        raise HTTPException(503, f"├ëchec g├⌐n├⌐ration PDF : {detail}") from exc
    finally:
